 ...
 *
 * Pustaka Inspirasi Minang compatibility override for OJS 3.5.
 *
 * OJS 3.5 generates the Editorial Masthead from active user-role assignments,
 * while the legacy/free-text editorial team content is stored in the journal's
 * `editorialHistory` setting. PIM journals manage their published editorial
 * team through that rich-text field, so prefer it when present. If it is empty,
 * fall back to the stock OJS 3.5 generated masthead.
 *}
...
<div class="page page_masthead">
	...
	{assign var="pimEditorialContent" value=$currentContext->getLocalizedData('editorialHistory')}

	{if !empty($pimEditorialContent)}
		<div class="pim_editorial_masthead_content">
			{$pimEditorialContent|strip_unsafe_html}
		</div>
	{else}
		{* Stock OJS 3.5 behaviour as a safe fallback when no static content exists. *}
